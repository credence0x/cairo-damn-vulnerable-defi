...
mod test_side_entrance_lender_pool {
    ...
    fn __setup_contracts() -> (IERC20Dispatcher, ISideEntranceLenderPoolDispatcher) {
        ...
        let mut ether_token = IERC20Dispatcher{ 
            contract_address: address_ether_token 
        };
        ...
        let (address_side_pool, _) = deploy_syscall(
            side_entrance_lender_pool::TEST_CLASS_HASH.try_into().unwrap(), 0, calldata.span(), false
        ).unwrap();
        let side_pool = ISideEntranceLenderPoolDispatcher{ 
            contract_address: address_side_pool 
        };        
    ...
    fn __fund_pool(ether_token: IERC20Dispatcher, side_pool: ISideEntranceLenderPoolDispatcher) {
        ...
        ether_token.transfer(
            side_pool.contract_address,
            __ether_in_pool()
        );
    }
    ...
    fn __check_solution(
        ...
        assert_eq(
            @ether_token.balance_of(side_pool.contract_address),
            @0,
            'Pool should have 0 Ether'
        );
    }
}
