...
mod test_uniswap_factory {
    ...
    fn get_exchange() {
        let (uniswap_factory, uniswap_exchange, _, damn_valuable_token) = __setup_factory();

        let exchange_address = uniswap_factory.get_exchange(damn_valuable_token.contract_address);

        assert_ne(@exchange_address.into(), @0, 'Exchange address cant be zero');

        assert_eq(
            @exchange_address, @uniswap_exchange.contract_address, 'Exchange address is not correct'
        );
    }
    ...
    fn get_token() {
        ...
        let token_address = uniswap_factory.get_token(uniswap_exchange.contract_address);

        assert_ne(@token_address.into(), @0, 'Token address cant be zero');

        assert_eq(
            @token_address, @damn_valuable_token.contract_address, 'Token address is not correct'
        );
    }
    ...
    fn panic_initialize_factory() {
        let (uniswap_factory, _, ether_token, _) = __setup_factory();
    ...
    fn panic_create_exchange() {
        let (uniswap_factory, _, _, damn_valuable_token) = __setup_factory();
    ...
    fn __setup_factory() -> (
        IUniswapFactoryDispatcher, IUniswapExchangeDispatcher, IERC20Dispatcher, IERC20Dispatcher
    ) {
        ...
        let uniswap_exchange_address = uniswap_factory
        ...
        (uniswap_factory, uniswap_exchange, ether_token, damn_valuable_token)
    }
}
