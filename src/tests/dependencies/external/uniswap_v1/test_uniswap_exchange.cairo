...
mod test_dvt_exchange {
    ...
    use damnvulnerabledefi::tests::utils::constants::{DEPLOYER, RECEIVER};
    ...
    fn test_factory_address() {
        let (uniswap_factory, ether_token, damn_valuable_token, other_side_token) =
            __setup_exchange();
    ...
    fn test_name_symbol_decimal() {
        ...
        assert_eq(@dvt_exchange.name(), @'Uniswap V1', 'WRONG_NAME');

        assert_eq(@dvt_exchange.symbol(), @'UNI-V1', 'WRONG_SYMBOL');

        assert_eq(@dvt_exchange.decimals(), @damn_valuable_token.decimals(), 'WRONG_DECIMALS');
    }
    ...
    fn test_add_liquidity() {
        let (uniswap_factory, ether_token, damn_valuable_token, _) = __setup_exchange();
        ...
        ether_token.approve(dvt_exchange.contract_address, eth_amount);
        damn_valuable_token.approve(dvt_exchange.contract_address, max_tokens);
        let DVT_UNI = dvt_exchange
            .add_liquidity(
                eth_amount, to_wad(0), // immaterial when total Liquidity is 0
                 max_tokens, 12
            );

        assert_eq(@DVT_UNI, @to_wad(200), // 200 DVT_UNI
        ...
        let DVT_UNI = dvt_exchange
            .add_liquidity(
                eth_amount, to_wad(1), // immaterial when total liquidity is 0
                 max_tokens, 1
            );

        assert_eq(@DVT_UNI, @to_wad(3), // 3 DVT_UNI
         'WRONG DVT_UNI AMOUNT');
        ...
        assert_eq(@DVT_UNI, @to_wad(12), // 12 DVT_UNI
         'WRONG DVT_UNI AMOUNT');
    }
    ...
    fn test_get_eth_to_token_input_price() {
        let (uniswap_factory, _, damn_valuable_token, _) = __setup_with_initial_liquidity();
    ...
    fn test_eth_to_token_swap_input() {
        let (uniswap_factory, ether_token, damn_valuable_token, _) =
        ...
        ether_token.approve(dvt_exchange.contract_address, eth_sold);
        let tokens_bought = dvt_exchange.eth_to_token_swap_input(eth_sold, min_tokens, deadline);
    ...
    fn test_eth_to_token_transfer_input() {
        ...
        ether_token.approve(dvt_exchange.contract_address, eth_sold);
        let tokens_bought = dvt_exchange
            .eth_to_token_transfer_input(eth_sold, min_tokens, deadline, RECEIVER());
    ...
    fn test_eth_to_token_swap_output() {
        ...
        ether_token.approve(dvt_exchange.contract_address, max_eth);
        let eth_sold = dvt_exchange.eth_to_token_swap_output(max_eth, tokens_bought, deadline);

        assert_eq(@eth_sold, @max_eth, 'WRONG_ETH_SOLD_VALUE');
    ...
    fn test_eth_to_token_transfer_output() {
        ...
        ether_token.approve(dvt_exchange.contract_address, max_eth);
        let eth_sold = dvt_exchange
            .eth_to_token_transfer_output(max_eth, tokens_bought, deadline, RECEIVER());
    ...
    fn test_token_to_eth_swap_input() {
        ...
        damn_valuable_token.approve(dvt_exchange.contract_address, tokens_sold);
        let eth_bought = dvt_exchange.token_to_eth_swap_input(tokens_sold, min_eth, deadline);

        assert_eq(@eth_bought, @min_eth, 'WRONG_ETH_BOUGHT_VALUE');
    ...
    fn test_token_to_eth_transfer_input() {
        ...
        damn_valuable_token.approve(dvt_exchange.contract_address, tokens_sold);
        let eth_bought = dvt_exchange
            .token_to_eth_transfer_input(tokens_sold, min_eth, deadline, RECEIVER());
    ...
    fn test_token_to_eth_swap_output() {
        ...
        damn_valuable_token.approve(dvt_exchange.contract_address, max_tokens);
        let tokens_sold = dvt_exchange.token_to_eth_swap_output(eth_bought, max_tokens, deadline);

        assert_eq(@tokens_sold, @max_tokens, 'WRONG_TOKENS_SOLD_VALUE');
    ...
    fn test_token_to_eth_transfer_output() {
        ...
        damn_valuable_token.approve(dvt_exchange.contract_address, max_tokens);
        let tokens_sold = dvt_exchange
            .token_to_eth_transfer_output(eth_bought, max_tokens, deadline, RECEIVER());
    ...
    fn token_to_token_swap_input() {
        ...
        let ost_exchange = IUniswapExchangeDispatcher {
            contract_address: uniswap_factory.get_exchange(other_side_token.contract_address)
        };
        ...
        damn_valuable_token.approve(dvt_exchange.contract_address, dvt_tokens_sold);
        let ost_bought = dvt_exchange
    ...
    fn token_to_token_swap_output() {
        ...
        damn_valuable_token.approve(dvt_exchange.contract_address, max_dvt_tokens_sold);
        let tokens_sold = dvt_exchange
        ...
        assert_eq(@tokens_sold, @max_dvt_tokens_sold, 'WRONG_TOKENS_SOLD_VALUE');
    ...
    fn token_to_token_transfer_output() {
        ...
        damn_valuable_token.approve(dvt_exchange.contract_address, max_dvt_tokens_sold);
        let tokens_sold = dvt_exchange
            .token_to_token_transfer_output(
                ost_bought,
                max_dvt_tokens_sold,
                max_eth_sold,
                deadline,
    ...
    fn test_remove_liquidity() {
        ...
        let (eth_amount, token_amount) = dvt_exchange
            .remove_liquidity(dvt_UNI, min_eth, min_tokens, deadline);

        assert_eq(@eth_amount, @min_eth, 'WRONG_ETH_AMOUNT');

        assert_eq(@token_amount, @min_tokens, 'WRONG_TOKEN_AMOUNT');
    ...
    fn token_to_exchange_transfer_input() {
        ...
        damn_valuable_token.approve(dvt_exchange.contract_address, dvt_tokens_sold);
        let ost_bought = dvt_exchange
            .token_to_exchange_transfer_input(
                dvt_tokens_sold,
                min_ost_bought,
                min_eth_bought,
                deadline,
    ...
    fn token_to_exchange_transfer_output() {
        ...
        damn_valuable_token.approve(dvt_exchange.contract_address, max_dvt_tokens_sold);
        let tokens_sold = dvt_exchange
            .token_to_exchange_transfer_output(
                ost_bought,
                max_dvt_tokens_sold,
                max_eth_sold,
                deadline,
    ...
    fn __setup_exchange() -> (
        IUniswapFactoryDispatcher, IERC20Dispatcher, IERC20Dispatcher, IERC20Dispatcher
    ) {
        call_contracts_as(DEPLOYER());
        ...
        let other_side_token = IERC20Dispatcher { contract_address: other_side_token_address };
        ...
        ////////////////////////////////////////
        uniswap_factory.create_exchange(damn_valuable_token.contract_address);

        ////////////////////////////////////////
        // Create Other Side Token Exchange
        ////////////////////////////////////////
        uniswap_factory.create_exchange(other_side_token.contract_address);

        (uniswap_factory, ether_token, damn_valuable_token, other_side_token)
    }
    ...
    fn __setup_with_initial_liquidity() -> (
        ...
        damn_valuable_token.approve(dvt_exchange.contract_address, max_tokens);
        dvt_exchange
            .add_liquidity(eth_amount, 0, // immaterial when total Liquidity is 0
             max_tokens, 12);
        ...
        dvt_exchange.add_liquidity(eth_amount, to_wad(1), max_tokens, 1);
        ...
        ether_token.approve(ost_exchange.contract_address, eth_amount);

        other_side_token.approve(ost_exchange.contract_address, max_tokens);

        let OST_UNI = ost_exchange
            .add_liquidity(eth_amount, 0, // immaterial when total liquidity is 0
             max_tokens, 1);
