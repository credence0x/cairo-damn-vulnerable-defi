...
mod puppet_pool {
    ...
    use damnvulnerabledefi::contracts::dependencies::external::uniswap_v1::uniswap_interfaces::{
        IUniswapExchangeStorage, IUniswapExchangeStorageDispatcher, IUniswapExchangeStorageDispatcherTrait,
    };
    ...
    impl PuppetPoolPrivate of PuppetPoolPrivateTraits {
        ...
        fn receive_ether(ref self: ContractState, from: ContractAddress, value: u256){
            ...
            assert(ether_token.allowance(
                    from, 
                    starknet::get_contract_address()
                ) >= value,'INSUFFICIENT_ETHER_ALLOWANCE'
            );
    ...
    impl PuppetPool of super::IPuppetPool<ContractState> {
        fn borrow(
            ...
            let deposit_required = PuppetPool::calculate_deposit_required(
                ref self,
                amount
            );

            let caller = starknet::get_caller_address();
            PuppetPoolPrivate::receive_ether(
                ref self, 
                caller, 
                deposit_required
            );
            ...
            self.deposits.write(
                caller, 
                self.deposits.read(caller) + deposit_required
            );

            // Fails if the pool doesn't have enough tokens in liquidity
            assert(self.token.read().transfer(
                recipient, 
                amount
            ), 'TOKEN_TRANSFER_FAILED');


            self.emit(Event::Borrowed(
                Borrowed{
                    account: caller,
                    recipient: recipient,
        ...
        fn calculate_deposit_required(ref self: ContractState, amount: u256) -> u256 {
            ...
            amount * oracle_price * PuppetPoolConstant::DEPOSIT_FACTOR() / PuppetPoolConstant::ONE_WAD()
        }
    }
}
